...
const q = [
  ...
  },
  {
    ...
    title: () => isMinor()
      ? "Acceptez-vous que les réponses de ce bilan soient utilisées pour des statistiques anonymisées du séminaire ?"
      : "Acceptez-vous que vos réponses soient utilisées pour des statistiques anonymisées du séminaire ?",
    help: () => isMinor()
      ? "La participation à la collecte est facultative. Le bilan fonctionne de la même façon en cas de refus. Pour un mineur, l’autorisation doit être donnée par un parent ou responsable légal. Aucune donnée nominative (nom, e-mail, téléphone ou date de naissance) n’est demandée."
      : "La participation à la collecte est facultative. Vous pouvez refuser sans conséquence : le bilan et son résultat resteront identiques. Aucune donnée nominative (nom, e-mail, téléphone ou date de naissance) n’est demandée.",
    type: "single",
    options: [
...
function isValid(question, answer) {
  ...
  if (question.type === "number") {
    if (!(typeof answer === "number" && !Number.isNaN(answer) && answer >= question.min && answer <= question.max)) return false;
    // Pour l’indice de plaque calculé, le nombre de surfaces avec plaque
    // ne peut jamais dépasser le nombre total de surfaces examinées.
    if (question.id === "plaque_total" && typeof state.answers.plaque_surfaces === "number" && state.answers.plaque_surfaces > answer) {
      alert("Le nombre total de surfaces examinées doit être supérieur ou égal au nombre de surfaces avec plaque.");
      return false;
    }
    return true;
  }
...
function computePlaque() {
  ...
  if (typeof p === "number" && typeof t === "number" && t > 0 && p <= t) return Math.min(100, Math.round((p / t) * 100));
  return null;
}
...
async function submitStudyResponse(level) {
  ...
  try {
    ...
    let response;
    try {
      response = await fetch(`${SUPABASE_URL}/rest/v1/questionnaire_responses`, {
        method: "POST",
        headers: {
